...
@Singleton
class NvidiaNimProvider @Inject constructor(
    ...
    override suspend fun complete(request: LLMRequest): LLMResponse {
        ...
        client.newCall(httpRequest).execute().use { response ->
            ...
            if (!response.isSuccessful) {
                throw IOException("NVIDIA NIM request failed: Code ${response.code} - $responseBody")
            }
            if (responseBody == null) {
                throw IOException("Empty response body from NVIDIA NIM provider")
            }
    ...
    override fun streamComplete(request: LLMRequest): Flow<String> = flow {
        try {
            ...
            emit("Error streaming NVIDIA NIM: ${e.localizedMessage}")
        }
    }
